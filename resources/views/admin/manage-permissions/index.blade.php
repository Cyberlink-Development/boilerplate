    ...
    <div>
        <div class="rounded overflow-hidden shadow bg-white w-full mb-[1rem]">
            <div class="px-6 py-2 border-b border-light-grey flex justify-between">
                <div class="font-bold text-xl">Permissions</div>
                @can('permissions_create')
                    <div>
                        <a href="{{route('permissions.create')}}" class="focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-xs px-4 py-2 mb-2">Create</a>
                    </div>
                @endcan
            </div>
        </div>
                    ...
                    <div class="border-b border-gray-200 shadow w-full">
                        <table class="divide-y divide-green-400 w-full cursor-default">
                            ...
                            <tbody class="bg-white divide-y divide-gray-300">
                                ...
                                @endphp
                                @foreach($data as $row)
                                    @if(compare_roles_permission_with_permission($row))
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-gray-900">
                                                    @can('permissions_show')
                                                        <a href="{{route('permissions.show',$row->id)}}" class="hover:underline">{{name_deformat($row->name)}}</a>
                                                    @else
